...
# Timer to trigger model retraining after a certain interval
if st.button("Start Retraining Timer"):
    st.write("Retraining Timer Started...")
    timer_interval = st.number_input(
        "Enter retraining interval (in seconds):", min_value=1)
    while True:
        st.text("Retraining the model...")
        history = model.fit(x_train, y_train, validation_data=(x_val, y_val), epochs=20, batch_size=64)
        st.text("Model Retraining Completed")
        st.text("Evaluating the model on test data...")
        score = model.evaluate(X_test, y_test, verbose=0)
        st.write("Test loss:", score[0])
        st.write("Test accuracy:", score[1])
        st.text(f"Waiting for {timer_interval} seconds before next retraining...")
